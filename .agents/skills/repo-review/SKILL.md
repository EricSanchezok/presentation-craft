...
- The installable skill resolves all required references and assets inside its own directory.
- Navigation and motion states remain directly reachable and reversible.
- Offline delivery retains one clear entry point and distinguishes structural proof from actual browser verification.
- Hypothetical example data and research plans stay labeled as such; third-party materials keep their own terms.

## Manual checks

### Project-specific (instantiated at seed time from this project's stack and known failure modes)

- Trace a slide from its canonical definition through notes, playback, print and packaging.
- Inspect representative rendered states: anchors, labels, line endpoints, semantic wrapping and intermediate emphasis.
- Test rapid navigation, reduced motion, dialog focus and interruption only where affected.
- Challenge rules that turn one example or aesthetic preference into a universal prescription.
