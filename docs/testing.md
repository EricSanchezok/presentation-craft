...
- `tests/core.test.mjs`: Node tests for navigation, standalone catalog creation, complete SVG target states, and offline packaging. Temporary fixtures belong to their tests and are removed afterward.
- `tests/browser.mjs`: Playwright against built example entry points, using `scripts/serve.mjs`; screenshots and PDFs go to ignored `artifacts/`.
- `examples/`: authored evaluation briefs and generated previews; source decks live inside the skill; built copies go to ignored `outputs/examples/`.
- `tests/assets.mjs`: a temporary independent project verifies imported SVG and CSS images through the real build and browser entry point.
- `scripts/check-skill.mjs` verifies the installable package reference boundary.
- The starter has its own TypeScript and Oxlint checks. No separate unit framework or CI tier is configured.
